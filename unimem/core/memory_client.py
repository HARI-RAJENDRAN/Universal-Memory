...
class MemoryClient:
    ...
    def __init__(
        ...
        self._llm_service = LLMService(
            llm_client=llm_client or LocalLLMClient(timeout_seconds=300)
        )
    ...
    def chat(self, query: str, user_id: str) -> str:
        ...
        cache_key = (str(user_id), str(query.strip().lower()))
        if cache_key in self._query_cache:
            return self._query_cache[cache_key]
            
        relevant_context = self.search(query=query, user_id=user_id)
        reply = self._llm_service.generate_contextual_response(
            query=query, 
            context_items=relevant_context,
            use_llm=self.config.use_llm
        )
        
        # Don't cache offline generic errors so it recovers natively
        if "[SYSTEM: Generation Engine Offline" not in reply:
            self._query_cache[cache_key] = reply
            
        return reply

    def cleanup_memory(self, user_id: str) -> int:
        """Manually trigger the garbage collection of decayed memory points."""
        return self._memory_service.cleanup_memory(user_id)
