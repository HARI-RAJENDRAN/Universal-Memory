...
class LLMService:
    ...
    def _is_simple_query(self, query: str) -> bool:
        ...
        return any(t.startswith(p) or p in t for p in simple_patterns)

    def generate_contextual_response(
        self, query: str, context_items: list[str], use_llm: bool = True
    ) -> str:
        """Builds a customized prompt with user context and invokes the LLM."""
        if not use_llm or self._is_simple_query(query):
            if not context_items:
                return "I don't have any specific memory saved about that."
            return "Based strictly on my internal memory matrices:\n- " + "\n- ".join(context_items)
