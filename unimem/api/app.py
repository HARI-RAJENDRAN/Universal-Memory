...
from unimem.api.schemas import AddRequest, AddResponse, ChatRequest, ChatResponse, ExplainResponse
...
def add_memory(body: AddRequest, db: Session = Depends(get_db)):
    ...
    service = MemoryService(session=db, config=MemoryConfig())
    try:
...
def explain(user_id: str, query: str, db: Session = Depends(get_db)):
    """Transparents scoring extraction logic per user request."""
    service = RetrievalService(session=db, config=MemoryConfig())
    if not user_id.strip() or not query.strip():
        raise HTTPException(status_code=400, detail="user_id and query must not be empty")
    
    try:
        explanations = service.explain_retrieval(query=query, user_id=user_id)
